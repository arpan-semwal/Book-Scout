...
const [libraries, setLibraries] = useState<Library[]>([]);
  ...
  const availableAmenities = ["WiFi", "AC", "Power Backup", "Drinking Water", "CCTV"];
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          const errorMessage = axiosError.response?.data?.message || "Kuch toh gadbad hai!";
          alert(errorMessage);
        } finally {
          setLoading(false);
        }
  ...
  return (
    <div className="p-4 md:p-8 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-extrabold text-gray-800">Owner Dashboard</h1>
          <p className="text-gray-500">Manage Your Libraries and track performance</p>
        </div>
        <button
        ...
        {stats.map((stat, index) => (
          <div key={index} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              ...
              <div className="text-4xl bg-gray-50 p-3 rounded-xl">{stat.icon}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <h2 className="text-xl font-bold text-gray-800 mb-6">Your Registered Libraries</h2>
      
      {libraries.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {libraries.map((lib) => (
            <div key={lib.id} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
              ...
              <h3 className="text-lg font-bold text-gray-800 mb-1">{lib.name}</h3>
              <p className="text-gray-500 text-sm mb-4 line-clamp-2">📍 {lib.address}</p>
              
              <div className="flex flex-wrap gap-2 mb-4">
                {lib.amenities?.map((amenity: string) => (
                  <span key={amenity} className="text-[10px] bg-gray-100 text-gray-600 px-2 py-1 rounded-lg">
                    {amenity}
                  </span>
                ))}
              </div>
        ...
        <div className="bg-white rounded-3xl border-2 border-dashed border-gray-200 p-12 text-center">
          ...
          <h2 className="text-xl font-bold text-gray-700">No Libraries Found</h2>
          <p className="text-gray-500 mt-2">Pehli library add karne ke liye upar diye gaye button par click karein.</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 md:p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-800">Register New Library</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 text-3xl">&times;</button>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                <label className="block text-sm font-semibold mb-1 text-gray-700">Library Name</label>
                <input
                  required
                  type="text"
              ...
              <div>
                <label className="block text-sm font-semibold mb-1 text-gray-700">Full Address</label>
                <textarea
                  required
